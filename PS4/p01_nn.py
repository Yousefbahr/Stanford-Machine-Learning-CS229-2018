...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    # *** START CODE HERE ***
    grad_loss_wrt_W = forward_convolution(
        output_grad[:, np.newaxis, ...], # the original shape with dim of 1 added
        np.zeros(conv_W.shape[0]), data).reshape((2, 1, 4, 4))

    grad_loss_wrt_b = np.zeros_like(conv_b)
    for i in range(conv_W.shape[0]):
        grad_loss_wrt_b[i] = np.sum(output_grad[i, :, :])

    # perform 'same' convolution (add padding) between 'output_grad' and flipped 'conv_W', to make sure the gradient
    # wrt to the data is equal to the dimension of the input data

    # padding should make dimensions of 'grad_wrt_data' equal to dimensions of 'data'
    padding = conv_W.shape[2] - 1

    grad_loss_wrt_data  = np.pad(data, ((0, 0), (padding, padding), (padding, padding)), mode='constant', constant_values=0).astype("float64")

    flipped_W = np.flip(conv_W, axis=(2,3))

    conv_channels, _, conv_width, conv_height = flipped_W.shape # 2, 1, 4, 4

    input_channels, input_width, input_height = output_grad.shape

    for i in range(conv_channels):
        for j in range(input_height):
            for k in range(input_width):
                grad_loss_wrt_data[:, j: (j+conv_height), k: (k+conv_width) ] += output_grad[i, j, k] * flipped_W[i]

    # remove padding
    grad_loss_wrt_data = grad_loss_wrt_data[:, padding:-padding, padding:-padding]

    return grad_loss_wrt_W, grad_loss_wrt_b, grad_loss_wrt_data
...
def backward_max_pool(data, pool_width, pool_height, output_grad):
    ...
    # *** START CODE HERE ***
    grad_wrt_data = np.zeros_like(data)

    input_channels, output_width, output_height = output_grad.shape

    # Iterate over each pooling window in the output
    for k in range(input_channels):
        for i in range(output_width):
            for j in range(output_height):
                # Define the current pooling window in the input
                w_start = i * pool_width
                w_end = w_start + pool_width
                h_start = j * pool_height
                h_end = h_start + pool_height

                # Get the slice of input corresponding to the current pooling window
                pool_region = data[k, w_start:w_end, h_start:h_end]

                # Find the index of the maximum value in the pooling window
                max_index = np.unravel_index(np.argmax(pool_region, axis=None), pool_region.shape)

                grad_wrt_data[k, w_start:w_end, h_start:h_end][max_index] = output_grad[k, i, j]

    return grad_wrt_data
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params):
    ...
    gradients = {"W1": np.zeros_like(W1), "b1": np.zeros_like(b1), "W2": np.zeros_like(W2), "b2": np.zeros_like(b2)}
    ...
    flattened = np.reshape(first_after_relu, (-1))
    logits = forward_linear(W2, b2, flattened)
    output_probab = forward_softmax(logits)

    # Backpropagation

    # Gradient of the loss wrt to the output
    grad_cross_entropy_loss = backward_cross_entropy_loss(output_probab, labels)

    # Gradient wrt to the logits
    grad_softmax = backward_softmax(logits , grad_cross_entropy_loss)

    # Gradient wrt to the parameters of linear layer
    gradients["W2"], gradients["b2"], grad_linear = backward_linear(W2, b2, flattened, grad_softmax)

    # Gradient wrt to the input of RELU
    grad_relu = backward_relu(first_max_pool,grad_linear.reshape(first_max_pool.shape))

    # Gradient wrt to the input of max pool
    grad_max_pool = backward_max_pool(first_convolution, MAX_POOL_SIZE, MAX_POOL_SIZE, grad_relu)

    # Gradient wrt to the parameters of conv layer
    gradients["W1"], gradients["b1"], grad_conv_layer = backward_convolution(W1, b1, data, grad_max_pool)
...
def nn_test(data, labels, params):
    output, cost = forward_prop(data, labels, params) # it was forward_pr -> edited it to forward_prop
